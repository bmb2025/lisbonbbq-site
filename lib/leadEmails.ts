// Shared email helpers for lead notifications (used by api/save-lead.ts and
// api/notify-partial-leads.ts). Pure template functions — no external clients.

export const FROM = "LisbonBBQ <noreply@lisbonbbq.pt>";
export const NOTIFY_EMAIL = "pitmasters@lisbonbbq.pt";

export function formatDate(iso: string | undefined, lang: "pt" | "en" = "pt") {
  if (!iso) return "—";
...
export function traditionLabel(t: string, lang: "pt" | "en" = "pt") {
...
// Internal notification to the LisbonBBQ team. When `incomplete` is true the lead
// abandoned the form before finishing (only the data collected so far is shown).
export function internalEmail(lead: any, opts: { incomplete?: boolean } = {}) {
...
export function confirmationEmail(lead: any, lang: "pt" | "en" = "pt") {
  const b = lead.booking || {};
  const c = lead.client || {};
