...
export function fetchShowcaseEvents(limit = 6): Promise<ShowcaseEvent[]> {
  return rest<ShowcaseEvent[]>(
    `events?select=id,showcase_name,showcase_blurb,guest_count,showcase_images,venue_name,starts_at&published=eq.true&showcase=eq.true&order=starts_at.desc&limit=${limit}`
  );
}
